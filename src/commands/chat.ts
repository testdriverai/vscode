...
// Import ansi-to-html for rendering ANSI codes
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
async function handleChatMessage(userMessage: string, panel: vscode.WebviewPanel | vscode.WebviewView, context: vscode.ExtensionContext) {
  try {
    ...
    try {
      ...
      const cliArgs = {
        command: 'edit',
        args: [path.join(workingDir, 'testdriver', 'testdriver.yaml')], // Set path to default test file
        options: {
      ...
      // Listen to various events from the agent's emitter
      agent.emitter.on('status', (data: string) => {
        const webview = 'webview' in panel ? panel.webview : panel;
        webview.postMessage({
          command: 'status',
      ...
      agent.emitter.on('log:*', (data: string) => {
        // Skip debug events
        if (agent.emitter.event === 'log:debug') {
          return;
        }

        console.log('Log event received:', { event: agent.emitter.event, data });
        ...
        webview.postMessage({
          command: 'log',
          data: convert.toHtml(message)
        });
      });

      // Handle markdown events specifically for code blocks
      agent.emitter.on('log:markdown:*', (data: string) => {
        console.log('Markdown event received:', { event: agent.emitter.event, data });
        const webview = 'webview' in panel ? panel.webview : panel;
        webview.postMessage({
          command: 'markdown',
          data: data  // Send raw markdown content, not converted to HTML
        });
      });

      // Also listen for general markdown events
      agent.emitter.on('markdown', (data: string) => {
        console.log('General markdown event received:', data);
        const webview = 'webview' in panel ? panel.webview : panel;
        webview.postMessage({
          command: 'markdown',
          data: data
      ...
      agent.emitter.on('error:*', async (errorMessage: string) => {
        ...
        // Convert ANSI codes to HTML for better rendering
        const htmlErrorMessage = convert.toHtml(errorMessage);

        const webview = 'webview' in panel ? panel.webview : panel;
        webview.postMessage({
          command: 'error',
          data: htmlErrorMessage
        });
      ...
      // Open the test file being edited
